...
import { stockInProducts, stockInVendors, StockInProduct, StockInItem } from "./data";

export default function StockInPage() {
  const [poReqNo, setPoReqNo] = useState("");
  ...
  const [selectedVendor, setSelectedVendor] = useState<typeof stockInVendors[0] | null>(null);
  ...
  // Filter products
  const filteredProducts = stockInProducts.filter(
    (p) =>
  ...
  // Filter vendors
  const filteredVendors = stockInVendors.filter(
    (v) =>
  ...
  // Select product
  const handleProductSelect = (product: StockInProduct) => {
  ...
  const handleAddItem = () => {
    ...
    if (existing) {
      ...
      setItems((prev) => [
        ...prev,
        { id: Date.now().toString(), product: selectedProduct, grnQty: qty },
      ]);
    }
  ...
  const handleQtyChange = (itemId: string, newQty: string) => {
    ...
    if (newQty === "" || (!isNaN(qty) && qty >= 0)) {
      setItems((prev) =>
        prev.map((i) =>
          i.id === itemId ? { ...i, grnQty: newQty === "" ? 0 : qty } : i
        )
      );
    }
  };
  ...
  // Select vendor
  const handleVendorSelect = (vendor: typeof stockInVendors[0]) => {
  ...
  const totalQty = items.reduce((sum, i) => sum + i.grnQty, 0);
  ...
  return (
    <div className="flex gap-4 p-4 h-full">
      ...
      <div className="flex-[7] flex flex-col gap-4">
        ...
        {/* PO + Stock Transfer + Product Row */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 px-6 py-4 flex flex-col gap-4">
          {/* PO + Stock Transfer */}
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              <label className="text-sm text-gray-700 font-medium whitespace-nowrap">PO Req No.</label>
              <input
                type="text"
                value={poReqNo}
                onChange={(e) => setPoReqNo(e.target.value)}
            ...
            <div className="flex items-center gap-3">
              <label className="text-sm text-gray-700 font-medium whitespace-nowrap">Stock Transfer</label>
              <input
                type="text"
                value={stockTransfer}
                onChange={(e) => setStockTransfer(e.target.value)}
                className="w-48 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
              <button className="text-gray-400 hover:text-gray-600">
                <Search className="w-5 h-5" />
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#3d9a7e] text-white">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold w-32">GRN Qty.</th>
                </tr>
              </thead>
              <tbody>
                {items.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => setIsCessEnabled(e.target.checked)}
              className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <label className="text-sm text-gray-700">Is Cess Enabled</label>
          </div>
          ...
          {/* Bill No */}
          <div>
            <label className="block text-sm text-gray-700 font-medium mb-1">Bill No</label>
            <input
              type="text"
              value={billNo}
