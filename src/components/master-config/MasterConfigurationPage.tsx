...
import { Save, Loader2, Building2, Receipt, Package, Printer, Globe, Settings, ChevronDown, ChevronUp, Plus } from "lucide-react";
    ...
    setSections((prev) =>
      prev.map((s) =>
        s.id === sectionId && s.items
          ? { ...s, items: [...s.items, name] }
          : s
      )
    );
  };

  const handleDeleteItem = (sectionId: string, idx: number) => {
    setSections((prev) =>
      prev.map((s) =>
        s.id === sectionId && s.items
                ...
                <div className="flex items-center gap-3">
                  {section.addButtonLabel && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 bg-purple-500 text-white rounded-md text-sm font-medium hover:bg-purple-600 transition-colors"
                    >
                      <Plus size={14} />
                      {section.addButtonLabel}
                    </button>
