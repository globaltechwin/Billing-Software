...
import { sampleStockIns, StockInRecord } from "./data";
...
export default function StockInListPage() {
  ...
  const [filteredData, setFilteredData] = useState<StockInRecord[]>(sampleStockIns);

  const vendorList = [...new Set(sampleStockIns.map((r) => r.vendorName))];

  // Handle view report
  const handleViewReport = () => {
    let result = [...sampleStockIns];
    if (startDate) {
    ...
    if (branch) {
      result = result.filter((r) => r.branchName === branch);
    }
    if (vendor) {
      result = result.filter((r) => r.vendorName === vendor);
    }
  ...
  const handleClear = () => {
    ...
    setVendor("");
    setSearchQuery("");
    setFilteredData(sampleStockIns);
    setCurrentPage(1);
  };
  ...
  const searchFilteredData = searchQuery
    ? filteredData.filter(
        (r) =>
          ...
          r.poRequestNo.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : filteredData;
    ...
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="bg-[#f2f5f9] px-6 py-3 flex items-center justify-between border-b border-gray-200">
          <h2 className="text-base font-semibold text-gray-800">Stock In List</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#3d9a7e] text-white">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold">PO REQUEST</th>
              </tr>
            </thead>
            <tbody>
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((record) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{record.billNo}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{record.vendorName}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{record.remarks}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{record.branchName}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{record.createdDate}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{record.poRequestNo}</td>
                  </tr>
                ))
              )}
            </tbody>
