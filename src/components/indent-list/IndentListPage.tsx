...
import { sampleIndents, IndentRecord } from "./data";
...
export default function IndentListPage() {
  ...
  const [filteredData, setFilteredData] =
    useState<IndentRecord[]>(sampleIndents);

  // Handle view report
  const handleViewReport = () => {
    let result = [...sampleIndents];
    if (startDate) {
  ...
  const searchFilteredData = searchQuery
    ? filteredData.filter(
        (r) =>
          ...
          r.indentType.toLowerCase().includes(searchQuery.toLowerCase()) ||
          r.indentTo.toLowerCase().includes(searchQuery.toLowerCase()) ||
                ...
                paginatedData.map((indent) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(indent.requestStatus)}`}
                      >
                        {indent.requestStatus}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
        ...
        <div className="px-4 py-3 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-semibold text-gray-700">
              {searchFilteredData
                .reduce((sum, r) => sum + r.noOfProducts, 0)
                .toFixed(2)}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600">
              ...
              {Math.min(startIndex + entriesPerPage, searchFilteredData.length)}{" "}
