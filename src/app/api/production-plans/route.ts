...
import { generateProductionPlanNumber } from "@/lib/number-generators";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fromDate) where.requestDate = { ...((where.requestDate as Record<string, Date>) || {}), gte: toBusinessDate(fromDate) };
    if (toDate) where.requestDate = { ...((where.requestDate as Record<string, Date>) || {}), lte: toBusinessDate(toDate) };
    ...
    console.error("Production plan list error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const planItems = items.map((i: Record<string, unknown>) => {
    ...
    const planNo = await generateProductionPlanNumber(companyId);
    ...
    const plan = await prisma.$transaction(async (tx) => {
      const created = await tx.productionPlan.create({
        data: {
          ...
          planNo,
          productionCategory,
          requestDate: toBusinessDate(requestDate),
          remarks: remarks || null,
          numberOfProducts: planItems.length,
          createdByUserId: userId,
          updatedByUserId: userId,
          items: {
            create: planItems.map((i) => ({
              companyId,
              productId: i.productId,
              quantity: i.quantity,
              price: i.price,
    ...
    return NextResponse.json({ success: true, plan: serializePlan(plan) }, { status: 201 });
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!existing) return NextResponse.json({ error: "Production plan not found" }, { status: 404 });
    ...
    if (body.requestDate !== undefined) {
      if (!String(body.requestDate).trim()) return NextResponse.json({ error: "Request Date is required" }, { status: 400 });
      data.requestDate = toBusinessDate(String(body.requestDate).trim());
    }
    ...
    await prisma.$transaction(async (tx) => {
      if (Array.isArray(body.items)) {
        ...
        const planItems = body.items.map((i: Record<string, unknown>) => {
          const productId = parseInt(String(i.productId), 10);
          const product = products.find((p) => p.id === productId)!;
          const quantity = Number(i.quantity) || 1;
          const price = Number(i.price) > 0 ? Number(i.price) : Number(product.sellingPrice) || 0;
        ...
        await tx.productionPlanItem.deleteMany({ where: { productionPlanId: id } });
        await tx.productionPlanItem.createMany({
          data: planItems.map((i: { productId: number; quantity: number; price: number; totalPrice: number }) => ({
            productionPlanId: id,
            companyId,
            productId: i.productId,
            quantity: i.quantity,
        ...
        data.numberOfProducts = planItems.length;
      }

      await tx.productionPlan.update({ where: { id }, data });
    });
    ...
    return NextResponse.json({ success: true, plan: serializePlan(updated!) });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error("Production plan delete error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
