...
export default function WastageListPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      data = data.filter(
        (r) =>
          r.wastageNo.toLowerCase().includes(q) ||
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="bg-[#3d9a7e] text-white">
                ...
                <th className="px-3 py-3 text-center text-xs font-semibold">
                  <div className="flex items-center justify-center gap-1">WASTAGE NO. ↕</div>
                </th>
                <th className="px-3 py-3 text-center text-xs font-semibold">
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((row) => (
                  <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-3 py-3 text-sm text-gray-700 text-center font-medium">{row.wastageNo}</td>
