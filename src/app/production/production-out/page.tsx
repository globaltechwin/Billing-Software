...
export default function ProductionOutPage() {
  ...
  const [items, setItems] = useState<ProductionOutItem[]>([]);
  ...
  const handleProductSelect = (product: Product) => {
    ...
    if (exists) {
      ...
      const newItem: ProductionOutItem = {
  ...
  const handleAutoStockOut = () => {
    if (items.length === 0) {
      alert("Please add products first");
      return;
    }
    alert("Auto stock out applied!");
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/production-out", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          productionCategory,
          indentNo,
          branchId: branchOut || null,
          remarks,
          items: items.map((i) => ({
      ...
      if (!data.success) {
        alert(data.error || "Failed to save production out");
        return;
      }
      alert("Production Out saved successfully!");
      handleClear();
    } catch {
      alert("Failed to save production out");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 flex items-center justify-between flex-wrap gap-2">
              <h2 className="text-lg font-semibold text-gray-800">Production Out</h2>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 flex flex-wrap items-center gap-6">
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-gray-700 whitespace-nowrap">Indent No.</label>
                <div className="relative">
                  <input
                    type="text"
                    value={indentNo}
                    onChange={(e) => setIndentNo(e.target.value)}
                    className="w-[140px] px-3 py-1.5 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
              ...
              <table className="w-full min-w-[900px]">
                <thead>
                  <tr className="bg-[#3d9a7e] text-white">
                    ...
                    <th className="px-4 py-3 text-center text-xs font-semibold w-[120px]">Cur. Stock</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold w-[120px]">Qty</th>
                  ...
                  {items.length === 0 ? (
                    ...
                    items.map((item) => (
                      <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-700 text-center">{item.currentStock}</td>
                        <td className="px-4 py-3 text-center">
                          <input
                            type="number"
                            value={item.quantity}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-5 py-5 flex flex-col gap-4">
              <div className="flex flex-wrap items-center gap-2">
                <label className="text-sm font-medium text-gray-700 whitespace-nowrap">Branch Out</label>
                <select
                  value={branchOut}
                ...
                <button
                  onClick={handleAutoStockOut}
                  className="px-3 py-1.5 bg-billora-primary text-white rounded-md text-xs font-medium hover:bg-billora-primary-dark transition-colors whitespace-nowrap"
                >
                  AutoStockOut
                </button>
              </div>
